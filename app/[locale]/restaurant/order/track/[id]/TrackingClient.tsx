...
import type { OrderWithLines } from "@/lib/order-repo";
import type { Fulfillment, OrderStatus } from "@/lib/order-mutations";

/**
 * Live wrapper around the customer order receipt. Subscribes to a Supabase
 * Realtime channel scoped to this one order row; on any UPDATE we call
 * `router.refresh()` so the server component re-fetches and the timeline
 * picks up the new status.
 *
 * Polling fallback: if the realtime channel never reaches SUBSCRIBED (no
 * websocket, table not in publication, anon read policy missing — see
 * [db/migrations/0011_orders_realtime.sql](../../../../../db/migrations/0011_orders_realtime.sql)),
 * a 15-second `setInterval` keeps `router.refresh()` firing so the UI is
 * never more than 15 s stale. We don't strictly need to gate the interval
 * on channel state — `router.refresh()` is cheap — but skipping it when
 * realtime is healthy keeps the dev/network log quieter.
 *
 * All presentational code lives here too (it was server-rendered in Phase
 * 4 / 5; moving it into a client file lets us pass `order` as a fresh prop
 * from the parent server component on each refresh without duplicating the
...
export function TrackingClient({ order }: { order: OrderWithLines }) {
  ...
  useEffect(() => {
    ...
    // Fallback poll — only fires when the realtime channel isn't healthy.
    // 15 s matches the cadence called out in the original Phase 4 doc.
      ...
      >
        ...
        {!cancelled && (
          <section style={cardStyle}>
            ...
            >
              <strong style={{ color: "#C9A84C" }}>Live tracking:</strong> this
              page updates the moment the kitchen advances your order — no
              refresh needed.
            </p>
          </section>
        )}
